...
namespace BotBits.Permissions
{
    public static class GroupExtensions
    {
        ...
        {
            if (source.GetGroup() < group) throw new AccessDeniedCommandException(errorMessage);
        }

        public static void RequireFor(this Group group, Player player,
            string errorMessage = "You are not allowed to run this command!")
        {
            if (player.GetGroup() < group) throw new AccessDeniedCommandException(errorMessage);
        }
    }
}